...
> - Examples where inventory writes share the order's transaction do not apply: order and inventory are separate services that never share a transaction; placement uses reserve, save, compensate (`polluxkart-architecture`).
> - Never call a remote system (Razorpay, SES, S3, another service's interface) while a transaction is open, even where an example here does (`polluxkart-third-party-integration`).
