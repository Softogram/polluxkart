...
> - Migrations live per service schema at `src/main/resources/db/migration/<schema>/` in the owning module, not one shared folder, and run as the `pk_migrator` role (`polluxkart-postgres-jpa`).
> - Money columns are `bigint` paise, never `NUMERIC(...,2)`; seeders use paise too.
> - Never seed an admin account or any password, including in development; the first admin is created by a one-off command (`polluxkart-security-checklist`).
