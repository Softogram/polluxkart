...
> - **The PolluxKart colours, fonts and theme are fixed (owner decision, 2026-09-13).** Never run `--design-system` or `--persist` to choose a palette, style or font pairing, never create `design-system/` files, and ignore any palette or font this skill suggests. Follow `polluxkart-design`.
> - Use this skill for UX guidance only: `--domain ux`, and `--stack nextjs`, `--stack shadcn` or `--stack react` searches for layout, accessibility, interaction and performance patterns.
> - Step 2 below ("Generate Design System, REQUIRED") does not apply here.
> - Advice to deploy on Vercel or to avoid self-hosting does not apply: PolluxKart self-hosts Next.js in Docker on AWS.
> - Currency examples in dollars do not apply: money is integer paise, formatted for INR only at display (`polluxkart-commerce`).
> - The upstream `scripts/tests/` folder was removed when vendoring, because those tests search parent folders for scripts and run them.
