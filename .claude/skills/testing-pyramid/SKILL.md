...
> - Use PostgreSQL 18 (`postgres:18`) in Testcontainers, matching RDS, not `postgres:16-alpine`.
> - With Spring Boot 4.1 the Testcontainers BOM is 2.x, where the PostgreSQL module artifact is `org.testcontainers:testcontainers-postgresql`, not `org.testcontainers:postgresql`.
> - API error bodies are Problem Details with a stable `code` (`polluxkart-error-handling`), not a `success`/`error` wrapper.
> - The PolluxKart testing rules, including zero retries and no skipped tests, are in `polluxkart-testing`.
