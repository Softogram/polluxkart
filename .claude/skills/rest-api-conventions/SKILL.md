...
> - Use the single central error handler and response shape from `polluxkart-error-handling`; do not add this skill's `GlobalExceptionHandler` template alongside it.
> - URL, pagination, session and CSRF conventions are in `polluxkart-http-api`.
