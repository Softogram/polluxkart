...
> - Money is `long` paise in Java and `bigint` in PostgreSQL, never `BigDecimal` or `numeric`, and always INR (`polluxkart-postgres-jpa`).
> - Entities never reference another service's entities; across services, references are ids only with no foreign key (`polluxkart-architecture`).
> - Entities never leave their service's `internal` package, and ids are UUIDv7.
