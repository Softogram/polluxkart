...
> - PolluxKart's Problem Details never include `detail`, always include `code` and `requestId`, and put field problems in `errors[]` (`polluxkart-error-handling`). Treat this skill's handler template as background only.
